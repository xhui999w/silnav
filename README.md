...
- 无需进入编辑模式即可拖动排序；桌面直接按住拖动，手机短暂按住后拖动
- 桌面端在网址图标、文字或卡片任意位置点击鼠标右键，可编辑、删除，NAS 入口可选择内网/外网打开
- Docker 版将排序写入 NAS，内网/外网视图和所有设备共享同一顺序
- NAS 保存网址前自动保留最近 10 个历史版本，可在设置中恢复上一版本
- 自动获取的图标会压缩并保存到 NAS，不依赖各电脑的浏览器缓存
- “修复缺失图标”只补空白图标，不覆盖已有或自定义图标
- 多设备同时编辑时使用数据修订号阻止静默覆盖
- 清亮、深色、暖色三套主题
- 桌面、平板和手机响应式布局
